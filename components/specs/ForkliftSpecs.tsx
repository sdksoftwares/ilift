...
interface ProductProps {
    product: {
        specifications?: {
            ...
            load_center?: RangeValue | number
            customSpecs?: { key: string; value: string }[]
        }
        sub_category_forklift?: string
    }
}

export default function ForkliftSpecs({ product }: ProductProps) {
    const specs = product.specifications || {}
    const subType = product.sub_category_forklift?.replace('forklift_', '').toUpperCase() || 'STANDARD'
    ...
    return (
        <div className="space-y-8">
            <div className="flex items-center gap-4 pb-4 border-b border-slate-200">
                ...
                <div>
                    <h3 className="text-xl font-bold text-slate-900 uppercase">Forklift Specifications</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                ...
                {/* Dimensions & Power */}
                <div className="space-y-4">
                    <h4 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-2">Dimensions & Power</h4>

                    <SpecRow label="Fork Length" value={formatRange(specs.fork_length)} />
                    {specs.load_center && <SpecRow label="Load Center" value={formatRange(specs.load_center)} />}
